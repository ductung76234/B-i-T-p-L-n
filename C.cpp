...
void docFile(MatHang *danhSach, int *soLuong, char *fileName) {
    // Hàm ??c d? li?u t? file vào danh sách
    FILE *file = fopen(fileName, "r");
    if (file != NULL) {
        *soLuong = 0;
        while (fscanf(file, "%s %s %f %d", danhSach[*soLuong].maHang, danhSach[*soLuong].tenHang, &danhSach[*soLuong].gia, &danhSach[*soLuong].soLuong) != EOF) {
            (*soLuong)++;
        }
        fclose(file);
...
int main() {
    ...
    do {
        ...
        printf("8. Thong ke gia tri hang hoa\n");
        printf("0. Thoat\n");
        ...
        switch(luaChon) {
            ...
            case 6:
                ...
                break;
            case 7:
                docFile(danhSach, &soLuong, fileName);
                break;
            case 8:
